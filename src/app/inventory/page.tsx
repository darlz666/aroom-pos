...
import { listIngredientsAction, listStockInsAction, listSuppliersAction } from "@/lib/inventory/actions";
import { InventoryWorkspace } from "./inventory-workspace";

export default async function InventoryPage() {
  const actor = await requireInventoryManager();
  const [ingredients, suppliers, history] = await Promise.all([listIngredientsAction(), listSuppliersAction(), listStockInsAction()]);
  ...
  return <main lang="id" className="flex-1 bg-[#f6f4ef] p-4 text-[#292e28] sm:p-6">
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <InventoryWorkspace key={actor.id} actorId={actor.id} initialIngredients={ingredients} initialSuppliers={suppliers} initialHistory={history} />
    </div>
  </main>;
}
