      ...
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[#dedfd5] px-4 py-4 sm:px-6">
        ...
        {user.role === "ADMIN" && (
          <>
            <Link
              href="/inventory"
              className="inline-flex min-h-12 items-center rounded-lg border border-[#a8aea0] px-4 font-semibold focus-visible:outline-2 focus-visible:outline-offset-2"
            >
              Stock Management
            </Link>
        ...
        {canOperate && register.success && register.shift && (
          <SettlementPanel key={register.shift.id} shiftId={register.shift.id} />
        )}
