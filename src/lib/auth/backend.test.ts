...
test("authentication backend with isolated database and request cookies", async (t) => {
  ...
  const { requireUser, requireRole, requireOperator, requireInventoryManager } = await import("./authorization");
  ...
  try {
    await t.test("shift actions authenticate fresh users, derive ownership and survive logout/login", async () => {
      const originalTransaction = prisma.$transaction;
      const originalShiftRead = prisma.shift.findFirst;
      ...
      const tx = {
        shift: {
          ...
          create: async ({ data }: { data: Record<string, unknown> }) => {
            assert.deepEqual(Object.keys(data).sort(), ["cashierId", "openingCash", "status"]);
            shift = { ...data, id: "shift", openedAt: new Date(), cashier: { name: user.name } };
            return shift;
          },
        },
    ...
    await t.test("authorization guards use current database state for the same session", async () => {
      ...
      try {
        ...
        await assert.rejects(requireInventoryManager(), redirectsTo("/login"));
        await createSession(user.id);
        ...
        assert.deepEqual(await requireInventoryManager(), safeUser());
        user.role = "CASHIER";
        await assert.rejects(requireInventoryManager(), redirectsTo("/"));
        assert.deepEqual(await requireUser(), safeUser());
        ...
        for (const role of ["STOCK_MANAGEMENT", "FINANCE"] as const) {
          user.role = role;
          if (role === "STOCK_MANAGEMENT") assert.deepEqual(await requireInventoryManager(), safeUser());
          else await assert.rejects(requireInventoryManager(), redirectsTo("/"));
